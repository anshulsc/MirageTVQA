...
class Qwen25VLAttentionOutputExtractor:
    ...
    def _compute_attention_manually(self, layer, hidden_states, vision_token_indices, use_masked_attention):
        ...
        last_token_head_outputs = attn_output[0, :, last_token_idx, :]  # (num_heads, head_dim)
    ...
    def process_dataset(self, dataset, lang_a, lang_b):

        prompt_a = self.config.LANGUAGE_PROMPTS[lang_a]
        prompt_b = self.config.LANGUAGE_PROMPTS[lang_b]

        self.logger.info(f"Starting feature extraction for {lang_a} vs {lang_b}")
        self.logger.info(f"Prompt A ({lang_a}): {prompt_a}")
        self.logger.info(f"Prompt B ({lang_b}): {prompt_b}")
        ...
        for i in tqdm(range(len(dataset)), desc="Extracting features"):
            try:
                ...
                prompts = [(prompt_a, 1, lang_a), (prompt_b, -1, lang_b)]
                
                for prompt, label, lang in prompts:
                ...
                self.logger.warning(f"Failed to download image for sample {i} from URL {record.get('coco_url', 'N/A')}: {e}")
            except Exception as e:
                self.logger.error(f"A general error occurred on sample {i}: {e}", exc_info=True)
